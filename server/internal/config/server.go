...
func (s *Server) Bind(cmd *cobra.Command) error {
	...
	// Changed default path prefix from "/neko" to "/" so the app is accessible
	// at the root when running locally without a reverse proxy in front.
...
// Addr returns the host:port string used to listen for incoming connections.
// If Host is empty the server listens on all interfaces (e.g. ":3000").
